...
- **QA Tester (AI):** Checks logic, edge cases, and requirement fulfillment. Also runs unit, regression and integration tests, ensuring at least a 70% code coverage and that all tests are located in a dedicated directory. The QA tester **must also validate that running docker compose up does not error out** and that all the services specified start correctly
