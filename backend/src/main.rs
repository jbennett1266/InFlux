use axum::{
    ...
    Router, Json, extract::{Request, Path, Extension},
            ...
            move |Extension(user): Extension<CurrentUser>, Json(payload): Json<CreateThreadRequest>| {
                ...
                async move {
                    ...
                    for member in &members {
                        let thread_name = if !payload.is_group && members.len() == 2 {
                            // Find the OTHER member's name for this user's view
                            members.iter()
                                .find(|&&ref m| m != member)
                                .cloned()
                        ...
                        let _ = db.query("INSERT INTO influx_ks.user_threads (username, thread_id, thread_name, is_group) VALUES (?, ?, ?, ?)", (member, &thread_id, thread_name, payload.is_group)).await;
                    }
...
async fn main() {
    ...
    let js = setup_nats("influx_events", &nats_url, "influx.events.>").await;
    let scylla = setup_cassandra(&cassandra_url).await;
